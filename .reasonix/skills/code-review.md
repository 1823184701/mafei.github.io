---
name: code-review
description: Systematic code review — security, performance, maintainability, correctness, testing with severity levels
runAs: subagent
---
...
- [ ] **Public Repo Push Protection** — `.gitignore` 包含敏感文件模式（`.env`、`*.key`、`*.pem`、`config/credentials*`）；使用 `git diff --cached --check` / `git secrets` / `pre-commit hooks` 防止 token、API key、连接串等被提交；审查 diff 中不出现明文密码或凭据
...
- **Suggest a fix** — Offer a concrete alternative when possible
- **Ask, don't demand** — Use questions for subjective points
- **Acknowledge good work** — Call out clean solutions and thorough tests
...
| **Drive-by Reviews** | Leaving one vague comment and disappearing. |
| **Scope Creep Reviews** | Requesting unrelated refactors that should be separate PRs. |
| **Stale Reviews** | Letting PRs sit for days. Review within 24 hours or hand off. |
...
7. NEVER make it personal — review the code, never the coder
8. NEVER merge code that contains hardcoded secrets, tokens, API keys, or connection strings — block with `[CRITICAL]` and require a pre-commit hook or `.gitignore` fix first
